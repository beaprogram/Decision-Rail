...
- [x] A refund commits during a broker outage and its event is delivered in order after recovery — including across a real process boundary: the application is killed with the event pending, a new process starts while the broker is still unavailable, serves payments, and delivers it with its original identity once the broker returns.
...
- [x] The application starts and serves payments while the broker's name does not resolve, and delivery resumes without a second restart.
