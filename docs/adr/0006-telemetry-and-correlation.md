...
**Sampling is decided once.** At the request, and honoured by every later hop: a stored origin exists
only because its producing span was recorded, so delivery propagates `sampled` rather than re-deciding.
Re-deciding per hop is what produces traces with holes in them.
