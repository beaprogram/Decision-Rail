...
| Return evidence | A payment's returned total must equal the sum of its return operations, checked by the database at commit from whichever side was written — the return rows or the payment itself. | A denormalised total and the rows it summarises can only disagree if something wrote one without the other. Attaching the check to only one of the two writes left the other unguarded. |
