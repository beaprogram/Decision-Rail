...
| Return evidence | A payment's returned total must equal the sum of its return operations, checked by the database at commit at all three points a transaction can break it: inserting the payment, updating its totals, and inserting a return operation. | A denormalised total and the rows it summarises can only disagree if something wrote one without the other. Attaching the check to the writes someone happened to think of left the others unguarded — twice, before the rule was derived from the state instead. |
| Payment identity | A payment's id cannot change once the row exists, refused immediately by the database. | Every journal, return, event, audit record and stored idempotency response names it, and a deferred constraint that captured one id must still find that row at commit — renaming the row between the two was a way past the returned-total check. |
| Currency agreement | A payment is denominated in the currency its funding account holds, structurally: `payments(account_id, currency)` references `accounts(id, currency)`, with no `ON UPDATE` cascade. Returns and journals already agreed with their payment, so agreement now holds transitively across account, payment, return and journal. | An invariant the service enforces but the schema does not is true by convention. A cascade would silently redenominate committed payments, which is the corruption this prevents rather than a fix for it. |
