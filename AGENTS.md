...
- Refunds and post-capture reversal share one capped return budget. A payment can never credit back more than it captured, enforced on every write by a row-level CHECK. The equality between a payment's returned total and the sum of its return operations is checked by a deferred trigger whenever a return is recorded, not on every update to the payment; drift within the cap is what reconciliation detects. A return does not change the payment's status; the totals do.
