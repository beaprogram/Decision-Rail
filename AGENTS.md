...
- A payment's currency must equal its funding account's, enforced structurally by a composite foreign key with no ON UPDATE cascade. Reconciliation is read-only. It derives expectations from the ledger and the operations that wrote it, never repairs what it finds, and is never reported as clean for a partially examined population.
